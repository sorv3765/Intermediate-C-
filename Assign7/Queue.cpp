...
private:
        size_t qSize=0;                                         // current number of items in Queue
        Node <T> *qFront=nullptr;
        Node <T> *qBack=nullptr;
public:
 ...
 *************************************************/
template <class T>
ostream& operator<<(std::ostream &lhs, const Queue<T>& other)
{
        for (Node<T>* i=other.qFront; i !=nullptr; i->next)
        {
